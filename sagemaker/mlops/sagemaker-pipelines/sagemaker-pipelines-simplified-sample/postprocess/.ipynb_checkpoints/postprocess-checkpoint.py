...
if __name__ == "__main__":
    ...
    test_csv_path = os.path.join('/opt/ml/processing/input/data','test.csv')
    test = np.loadtxt(test_csv_path, delimiter=',')
    test_x = xgboost.DMatrix(test[:,1:])
    answer_y = test[:,0]
    
    thistime_train_model_pred_y = predict('/opt/ml/processing/input/thistime_train_model', test_x) # Do Not Chane 'opt/ml/...' for the consistency of the nexecuting otebook
    lasttime_train_model_pred_y = predict('/opt/ml/processing/input/lasttime_train_model', test_x) # Do Not Chane 'opt/ml/...' for the consistency of the nexecuting otebook
    ...
    eval_result_path = os.path.join('/opt/ml/processing/output', args.output_file) # DO NOT CHANGE '/opt/ml/processing/output' for the consistency of execution notebook
